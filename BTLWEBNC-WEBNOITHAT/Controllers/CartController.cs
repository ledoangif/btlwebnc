...
namespace BTLWEBNC_WEBNOITHAT.Controllers
{
    public class CartController : Controller
    {
        ...
        //    return RedirectToAction("GioHang");
        //}
        public IActionResult Thaydoi(int id, string action)
        {
            var sanPham = db.TblChiTietGioHang1s.SingleOrDefault(x => x.IdchiTietDonHang == id);
            if (sanPham != null)
            {
                ...
                else if (action == "-" && sanPham.SoLuong > 1)
                {
                    sanPham.SoLuong -= 1;
                }
                ...
                var tongTienTatCa = db.TblChiTietGioHang1s
                        .AsEnumerable() // Chuyển sang tính toán trên máy khách
                        .Sum(x => x.TongTien);
                return Json(new { soLuong = sanPham.SoLuong, tongTien = sanPham.TongTien, tongTienTatCa = tongTienTatCa });
            }
            return NotFound();
        }
